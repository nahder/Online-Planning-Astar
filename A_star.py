...
class A_star: 
    ...
    def online_plan_path(self, start, goal):
        current_position = self.position_to_index(*start)
        path = [current_position]
        visited = {current_position}  # Using a set for O(1) lookup time

        while current_position != self.position_to_index(*goal):
            neighbors = self.get_neighbors(Node(current_position))
            
            # If no available steps, we're stuck
            if not neighbors:
                print("No available steps from position:", current_position)
                return path

            # Calculate cost-to-come for each neighbor
            neighbor_costs = {n[0].position: n[1] + self.manhattan_heuristic(n[0].position, self.position_to_index(*goal)) for n in neighbors if n[0].position not in visited}
            
            # Sort neighbors by cost-to-come
            sorted_neighbors = sorted(neighbor_costs, key=neighbor_costs.get)
            
            # If we have exhausted all the neighbors, we're stuck
            if not sorted_neighbors:
                print("Stuck at position:", current_position)
                return path
            
            # Pick the lowest-cost neighbor
            next_position = sorted_neighbors[0]

            # We should ensure it's not a wall before proceeding
            if neighbor_costs[next_position] < 1000:  # not a wall
...
def main(): 
    ...
    test.display_grid()
    path = test.online_plan_path(start=(0.5,-1.5),goal=(.5,1.5))
    test.visualize_results(path)
    path = test.online_plan_path(start=(4.5,3.5),goal=(4.5,-1.5))
    test.visualize_results(path)
    path = test.online_plan_path(start=(-0.5,5.5),goal=(1.5,3.5))
    test.visualize_results(path)
